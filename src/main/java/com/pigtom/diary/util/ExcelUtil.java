...
public class ExcelUtil {
     ...
     */
    public static <T> void writeDataToOutputStream(List<T> data, Class<T> tClass, OutputStream outputStream)
            throws IOException,InstantiationException, IllegalAccessException{

        // Workbook is type of HSSFWorkBook
        Workbook book = new HSSFWorkbook();
        ExcelUtil.writeDataToBook(book, data, tClass);

        ExcelUtil.writeWorkbookToOutputStream(book, outputStream);
    }

    public static <T> void writeDataToBook(Workbook workbook, @NotNull List<T> data, Class<T> tClass)
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            int columnWidth = excelColumn.columnWidth();
            sheet.setColumnWidth(columnIndex, columnWidth);
            cell = row.createCell(columnIndex);
            cell.setCellStyle(titleStyle);
            setValue(cell,  excelColumn.value());
        }

        for (int i = 1; i <= data.size(); i++) {
            // 创建一行
            row = sheet.createRow(i);
            Object rowData = data.get(i - 1);
            for (Field field : fields) {
                ExcelColumn excelColumn = field.getAnnotation(ExcelColumn.class);
                if (excelColumn == null) {
                    continue;
                }
                int column = excelColumn.columnIndex();

                // create every row by field
